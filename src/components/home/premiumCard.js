...
export default function premiumCard({
  ...
  return (
    <div className={` rounded-lg bg-black/30 backdrop-blur-[6px] overflow-hidden border-2 ${type === 'Pro' ? 'border-red-400/30' : 'border-slate-500'} border-opacity-20 p-9 flex flex-col items-center h-full`}>
      <div className=" flex justify-start items-center w-full">
        ...
        {type === "Pro" && (
          <div className=" absolute right-9 top-9 rounded-lg border-2 border-yellow-400/50 text-sm py-1 px-7 text-yellow-400/60">
            Most popular
          </div>
        )}
      </div>
      <div className=" w-full mt-9">
        <h1 className=" text-8xl font-semibold">${duration === "MONTHLY" ? priceM : priceY}</h1>
