...
export default function InterviewDisplayCard({ index, title, date, time }) {
  return (
    <div className=" group relative w-full h-full flex flex-col items-center justify-center rounded-xl p-6 bg-gray-800/80">
      ...
      <h3 className=" text-base md:text-lg">Date: {new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</h3>
      <h3 className=" text-base md:text-lg">Time: {new Date(time).toLocaleTimeString()}</h3>
    </div>
  );
}
